...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, layer_norm=True):
        ...
        self.actor_logstd = nn.Parameter(0.5*torch.ones(1, num_outputs), requires_grad=True)
...
def ppo(args):
    ...
    for i_episode in range(args.num_episode):
        ...
        rewards = torch.as_tensor(batch.reward)
        values = torch.as_tensor(batch.value)
        masks = torch.as_tensor(batch.mask)
        actions = torch.as_tensor(batch.action)
        states = torch.as_tensor(batch.state)
        oldlogproba = torch.as_tensor(batch.logproba)
        
        returns = torch.zeros(batch_size, dtype=torch.float32)
        deltas = torch.zeros(batch_size, dtype=torch.float32)
        ...
        for i_epoch in range(int(args.num_epoch * batch_size / args.minibatch_size)):
            ...
            minibatch_newlogproba = network.get_logproba(minibatch_states, minibatch_actions).to(device)
            minibatch_advantages = advantages[minibatch_ind].to(device)
            minibatch_returns = returns[minibatch_ind].to(device)
            minibatch_newvalues = network._forward_critic(minibatch_states).flatten().to(device)
    ...
    return network, running_state, reward_record
...
def test_record(args):
    ...
    record_dfs.to_csv(joindir(RESULT_DIR, 'ppo-origin-record-{}.csv'.format(args.env_name)))
    torch.save({
        'model_state_dict': network.state_dict(),
        'normalized_state': normalized_state
    }, joindir(RESULT_DIR, 'ppo-origin-modelpara-{}.pth'.format(args.env_name)))
